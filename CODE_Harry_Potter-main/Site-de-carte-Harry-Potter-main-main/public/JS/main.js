...
async function showDeck(){
    ...
    apiData.forEach((element => {                                                               //On affiche les cartes dans la div
        ...
        <div class="deck">
            ...
            <h2>${element.name}</h2>
            <div class="seeProfile"><a href="profil.html?id=${element.slug}">Voir le profil</a></div>    
        </div>
        `;
    }))
}
...
async function displayTriHouse(triHouse){
    ...
    triHouse.forEach((element => {
        ...
        <div class = "triDeck">
            <img src = "${element.image}" alt = " ${element.name}"/>
            <h2>${element.name}</h2>
            <div class="seeProfil"><a href="profil.html?id=${element.slug}">Voir le profil</a></div>
        </div>
        `;   
    }))
